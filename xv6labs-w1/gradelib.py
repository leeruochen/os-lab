...
import sys, os, re, time, socket, select, subprocess, errno, shutil, random, string, json, signal
...
class QEMU(object):
    ...
    def __init__(self, *make_args):
        # Dynamically find an available port instead of hardcoding 26000
        if QEMU._GDBPORT is None:
            for port in range(26000, 26100):
                try:
                    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    # Try to bind to the port; if it fails, it's truly in use
                    s.bind(('127.0.0.1', port))
                    s.close()
                    QEMU._GDBPORT = port
                    break
                except socket.error:
                    time.sleep(0.1) # Add this tiny delay
                    continue
            
            if QEMU._GDBPORT is None:
                print("Could not find an available port for QEMU GDB.", file=sys.stderr)
                sys.exit(1)

        if options.verbose:
            show_command(("make",) + make_args)
        
        # Pass the specific port to the make command
        # This ensures QEMU actually uses the port we found
        cmd = ("make", "-s", "--no-print-directory", "GDBPORT=%d" % QEMU._GDBPORT) + make_args
    ...
    def kill(self):
        #if self.proc:
        #    self.proc.terminate()
        """Hard kill the QEMU process and its entire process group."""
        if self.proc:
            try:
                # Recommendation: Kill the entire process group (pgid) 
                # instead of just the parent process to prevent 'bleeding'.
                os.killpg(os.getpgid(self.proc.pid), signal.SIGKILL)
                self.proc.wait() # Ensure the OS acknowledges it's dead
            except OSError:
...
class Runner():
    ...
    def run_qemu(self, *monitors, **kw):
        ...
        def run_qemu_kw(target_base="qemu", make_args=[], timeout=180):
        ...
        try:
            ...
            try:
                if self.qemu:
                    self.qemu.kill() # Now uses the robust kill logic
                if self.gdb:
                    self.gdb.close()
            except Exception as e:
                print("Note: Shutdown encountered an issue: %s" % e)
